...
RSpec.describe Milktea::Container do
  ...
  describe "dynamic child resolution" do
    let(:dynamic_container_class) do
      Class.new(described_class) do
        direction :column
        child :dynamic_child, flex: 1
        child Class.new(Milktea::Model), flex: 1
    ...
    context "when using regular child" do
      subject(:container) { dynamic_container_class.new(use_special: false) }

      it { expect(container.children.first).to be_a(RegularChildModel) }
    end

    context "when using special child" do
      subject(:container) { dynamic_container_class.new(use_special: true) }

      it { expect(container.children.first).to be_a(SpecialChildModel) }
    end

    context "when invalid child type" do
      let(:invalid_container_class) do
        Class.new(described_class) do
          child :invalid_child
      ...
      subject(:container) { invalid_container_class.new }

      it { expect { container.children }.to raise_error(ArgumentError, /Child must be a Class or Symbol/) }
    end
  end
end
